...
app.get(`${ config.baseUrl }admin`, (req, res, next) => {
  ...
  res.send(adminPage({ ...pugVars, appName: config.appName, lang: req.translations }));
});
  ...
  if (req.url.endsWith('.json')) {
    ...
    res.json({
      ...
      config: {
        ...
        previewVideos: config.previewVideos,
        showGallery: config.showGallery,
      }
    });
  } else {
    if (!db.get(req.params.sid)) return next();
    res.send(downloadPage({ ...pugVars, appName: config.appName, lang: req.translations }));
  }
});
...
app.get(`${ config.baseUrl }files/:fid`, async (req, res, next) => {
  ...
  // Thumbnails
  if (req.params.fid.match(/\.(thumb)$/)) {
    try {
      let options = { height: 180 }
      const requestedImage = req.params.fid.split('.thumb')[0];
      const info = await store.info(requestedImage); // throws on 404
      const stream = fs.createReadStream(store.getFilename(requestedImage));
      const thumbnail = await imageThumbnail(stream, options);
      res.send(thumbnail);
    }
    catch (e) {
